...
[NOTE]
The MDR is configured with Java class and field names, rather than database table and attribute names.

There are 3 phases to configure a new data source for the MDR.

.Phase 1
The first thing that should happen is to create a Mapping of the FURTHER model to your local data model.
...
You should have a good understanding of the following MDR data model before proceeding.
...
The Excel File's Instructions Tab contains these following steps:

1. Create a namespace - a namespace is itself an Asset of Asset type namespace (see other namespace Assets for examples. 
For demonstration purposes we will call it MyNamespace.
Retrieve the newly created Asset ID for MyNamespace.
This Asset ID will be used to create your classes.
...
.Phase 3
Configure Asset Associations and Properties
